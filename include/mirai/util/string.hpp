...
#if MR_HAVE_INT128
MR_NODISCARD MR_API i128 stoi128(const string& s) mr_noexcept {
	i128 res = 0, f = 1;
	auto pt = s.begin();
	if (*pt == '-') f = -1, pt++;
	while (pt != s.end()) {
...
MR_NODISCARD MR_API string to_string(i128 v) mr_noexcept {
	if (v == 0) return "0";
	string res;
	bool f = v < 0;
	if (f) v = -v;
...
#endif
MR_NODISCARD inline string itos(ll x, ll base, ull minimum_width = 0) mr_noexcept {
	string res;
